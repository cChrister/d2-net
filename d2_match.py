...
class Tracker0(object):
    ...
    def update(self, img, pts, desc,id):
        ...
        if self.pts_prev is None or id%2==0:
            self.pts_prev = pts
            self.desc_prev = desc
...
t_err_average, R_err_average, d = compute.compute_err(args.data_path, frame_id, skip=1, k=k, interpolate=True)
with open(str(args.data_path)+"/" + args.pair + "_result.txt", 'w') as f:
        ...
        f.write("每次提取特征点和描述子时间 %.6f ms" % (extra_time / frame_id * 100) + '\n')
